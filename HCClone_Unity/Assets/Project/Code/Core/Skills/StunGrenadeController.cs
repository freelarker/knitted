...
public class StunGrenadeController : MonoBehaviour {
	...
	public void Throw(float time, Vector3 startPosition, Vector3 targetPosition, float height, Action<Vector3> callback) {
		...
		AnimationCurve cY = new AnimationCurve(new Keyframe(0f, startPosition.y, 0f, tangentY), new Keyframe(time * 0.5f, startPosition.y + height), new Keyframe(time, startPosition.y, -tangentY, 0f));
